...
/* write_string_with_font will write a null-terminated string starting at the
   ...
   shiftAmount: How many bits to shift up
   Returns the next-open column*/
int write_string_with_font_16(uint16_t board[], char inStr[], int startColumn, uint16_t fontData[][DEFAULT_CHAR_LEN], int dontUseFontWidths, int *fontWidths, int shiftAmount) {
  int index;
  int letterIndex;  /* The de-asciified letter index, so letter - 'A' */
  int nextOpen;
  int col;
  ...
  do {
    ...
    letterIndex = inStr[index] - ' ';//FONT_START_CHAR;
    if (dontUseFontWidths) {
      /* I think I get the below font data correct....
         If you get junk to the screen, this is the first thing I would check.*/
         revIndex = dontUseFontWidths -1;
      for (col = startColumn; col < (startColumn + dontUseFontWidths + 1) ; col++) {
        board[col] = (fontData[letterIndex][revIndex--]);
      }
      /* Has the possibility to be off by one. Don't think so, but don't
         quote me.*/
      startColumn = startColumn + dontUseFontWidths;
    } else {
      /* I think I get the below font data correct....
         If you get junk to the screen, this is the first thing I would check.*/
      for (col = startColumn; col < (startColumn + dontUseFontWidths + 1) ; col++) {
        board[col] = (fontData[letterIndex][col - startColumn]);
      }
      /* Has the possibility to be off by one. Don't think so, but don't
         quote me.*/
      startColumn = startColumn + dontUseFontWidths;
...
void write_string_big(uint16_t board[], char inStr[]) {
  ...
  write_string_with_font_16(board, inStr, 0, DEFAULT_FONT, DEFAULT_CHAR_LEN, (int*)DEFAULT_FONT, 0);
}
